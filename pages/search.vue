          ...
          <div v-if="category === 'onlineVod'" class="p-2">
            <div class="space-y-4">
              ...
              <template v-else>
                <!-- 添加加载中的过渡提示 -->
                <div
                  v-if="Array.from(loadingStatus.values()).some((status) => status)"
                  class="fixed bottom-6 right-6 bg-gradient-to-r from-purple-500/90 to-blue-500/90 text-white px-4 py-2 rounded-lg shadow-lg backdrop-blur-sm z-20 flex items-center gap-3 transform hover:scale-105 transition-all duration-300"
                >
                  <div class="flex items-center gap-2">
                    <div class="relative">
                      <div class="w-4 h-4 rounded-full border-2 border-white/20 border-t-white animate-spin"></div>
                    </div>
                    <span class="text-sm font-medium">正在加载更多资源...</span>
                  </div>
                  <div class="text-xs px-2 py-0.5 bg-white/20 rounded-full">
                    {{ vodData.length }} 个结果
                  </div>
                </div>
                ...
                <vod-list
                  ...
                  :class="{
                    'opacity-80': Array.from(loadingStatus.values()).some(
                      (status) => status
                    ),
                  }"
                ></vod-list>
        ...
        <template v-if="isSearching">
          <div class="flex flex-col items-center justify-center gap-4">
            ...
            <div class="text-center">
              <p class="text-lg font-medium bg-gradient-to-r from-purple-600 to-blue-600 bg-clip-text text-transparent">
                正在搜索中...
              </p>
              <p class="text-sm mt-2 text-gray-500 dark:text-gray-400">请稍候片刻</p>
            </div>
          </div>
        </template>
...
// 添加监听器来处理搜索完成
watch([loadingProgress, loadingStatus], ([newLoadingProgress, newLoadingStatus]) => {
  if (category.value === 'clouddrive') {
    if (newLoadingProgress.completed >= newLoadingProgress.total) {
      isSearching.value = false;
    }
  } else if (category.value === 'onlineVod') {
    if (!Array.from(newLoadingStatus.values()).some(status => status)) {
      isSearching.value = false;
    }
  }
}, { deep: true })
