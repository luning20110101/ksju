...
const props = defineProps(['vodData'])
...
const handleVodTabChange = (index) => {
  vodTab.value = index
  const vod = props.vodData[index]
  isLoading.value = true
  
  // 获取该视频源的剧集列表
  const episodes = formatVodPlayUrl(vod)
  
  // 检查是否有可播放的剧集
  if (episodes.length === 0) {
    isLoading.value = false
    ElMessage({
      message: '当前视频源暂无可播放剧集',
      type: 'warning',
      duration: 3000,
    })
    return
  ...
  if (firstEpisode) {
    ...
    if (checkIsM3u8(firstEpisode.link)) {
      currentVod.value = {
        url: `${vod.playUrl}${firstEpisode.link}`,
        name: vod.vod_name
      }
    } else {
      currentVod.value = {
        url: firstEpisode.link,
        name: vod.vod_name
      }
    }
  }
      ...
      <div class="aspect-video w-full">
        ...
        <div v-if="!currentVod.url" 
          ...
          <div class="flex flex-col items-center gap-3">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">播放源和剧集加载中，请稍后...</p>
          </div>
        </div>
        
        <!-- 加载动画 -->
        <div v-if="isLoading && currentVod.url" 
          ...
          <div class="flex flex-col items-center gap-3">
            ...
            <span class="text-sm text-white/90 font-medium">加载中...</span>
          </div>
        </div>
    ...
    <div class="mt-6" v-if="vodData && vodData.length > 0">
      <div class="flex flex-wrap gap-2">
        <button 
          ...
          @click="handleVodTabChange(index)"
        >
